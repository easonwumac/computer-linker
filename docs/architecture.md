...
- CLI orchestration: `src/cli.ts`, `src/cli-help.ts`, `src/cli-format.ts`,
  ...
  - Long help text and help-topic routing live in `src/cli-help.ts` so
    `src/cli.ts` stays focused on command dispatch and command flows.

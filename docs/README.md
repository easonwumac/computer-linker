# Computer Linker Docs

Start with the smallest document that matches the job:

- [Getting Started](getting-started.md): install, expose one folder, connect an
  MCP client, and choose a tunnel.
- [Agent Instructions](agent-instructions.md): pasteable guidance for MCP
  clients and coding agents.
- [Client Recipes](client-recipes.md): local HTTP, tunnel, minimal client, and
  troubleshooting recipes.
- [API Compatibility](api-compatibility.md): public MCP contract and
  compatibility expectations.
- [Architecture](architecture.md): product model, runtime boundaries, and
  implementation module map.
- [Developer Guide](developer-guide.md): how to place new code, extend
  operations, and keep modules small.
- [Service Mode](service-mode.md): installed service profiles and smoke
  checklist.
- [Manual Test Plan](manual-test-plan.md): dogfooding checks before broader
  release.
- [Release Checklist](release-checklist.md): npm, public mirror, CI, audit, and
  tag workflow.
- [Product Spec](product-spec.md): product boundary and milestone definition.

For normal setup, use [Getting Started](getting-started.md). For development,
read [Architecture](architecture.md) and [Developer Guide](developer-guide.md)
before moving code across modules.

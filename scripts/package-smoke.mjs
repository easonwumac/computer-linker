...
assert(helpOutput.includes("computer-linker start <workspace-path> --dev --tunnel openai|tailscale|cloudflare"), "default CLI help must collapse tunnel providers into one first-run command");
assert(helpOutput.includes("computer-linker quickstart C:\\Projects\\my-app --dev"), "default CLI help must include the quickstart preview without exposing the full option matrix");
...
try {
  ...
  assert(installedHelp.includes("computer-linker quickstart C:\\Projects\\my-app --dev"), "installed CLI bin help did not expose quickstart preview");
  ...
  const installedQuickstart = parseJsonObjectOutput(
    runInstalledBin(consumerDir, "computer-linker", ["quickstart", installedWorkspaceDir, "--dev", "--json"]),
  ...
  const installedSelfTest = parseJsonObjectOutput(
    runInstalledBin(consumerDir, "computer-linker", ["self-test", "--json"]),
    "installed self-test",
  );
  assert(installedSelfTest.kind === "computer-linker-self-test", "installed self-test did not return the self-test report");

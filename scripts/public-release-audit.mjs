...
function collectTrackedFiles() {
  const output = runGit(["ls-files", "-z", "--cached"]);
  return output.split("\0").filter(Boolean).map(normalizePath);
}

function collectUntrackedFiles() {
  const output = runGit(["ls-files", "-z", "--others", "--exclude-standard"]);
  return output.split("\0").filter(Boolean).map(normalizePath);
}
...
const scanPatterns = [
  ...
  { id: "npm-token", regex: /npm_[A-Za-z0-9]{20,}/g },
...
const provenanceRiskPatterns = [
  { id: "copied-from", regex: /\b(?:copied|copy-pasted|copy pasted|borrowed|adapted|forked|vendored)\s+from\b/gi },
  { id: "based-on-url", regex: /\bbased\s+on\s+https?:\/\/[^\s)"'`<>]+/gi },
  { id: "derived-from-url", regex: /\bderived\s+from\s+https?:\/\/[^\s)"'`<>]+/gi },
  { id: "external-snippet-source", regex: new RegExp(`\\b(?:${["Stack\\s+Overflow", "gist\\.github\\.com", "codepen\\.io", "jsfiddle\\.net"].join("|")})\\b`, "gi") },
  { id: "third-party-code", regex: /\bthird[- ]party code\b/gi },
];

const legacyProductNamePatterns = [
  { id: "retired-product-name", regex: new RegExp(["dev", "space"].join(""), "gi") },
];
...
function scanFile(path, source) {
  ...
  for (const pattern of provenanceRiskPatterns) {
    pattern.regex.lastIndex = 0;
    for (const match of text.matchAll(pattern.regex)) {
      fail(`${source}:${path}:${lineNumberAt(text, match.index ?? 0)} matched provenance marker ${pattern.id}: ${compactMatch(match[0])}`);
    }
  }
  for (const pattern of legacyProductNamePatterns) {
    pattern.regex.lastIndex = 0;
    for (const match of text.matchAll(pattern.regex)) {
      fail(`${source}:${path}:${lineNumberAt(text, match.index ?? 0)} matched retired product-name marker ${pattern.id}: ${compactMatch(match[0])}`);
    }
  }
}
...
function checkLocalEvidenceFiles(trackedFiles, untrackedFiles, packedFiles) {
  const localEvidencePath = ".computer-linker-alpha-evidence.json";
  if (trackedFiles.includes(localEvidencePath)) {
  ...
  if (untrackedFiles.includes(localEvidencePath)) {
    fail(`${localEvidencePath} is unignored local dogfooding evidence and must not be present in a public release checkout; keep it gitignored and publish docs/alpha-evidence.example.json as the schema example`);
  }
...
for (const path of trackedFiles) fileSources.set(path, "tracked");
for (const path of untrackedFiles) fileSources.set(path, "untracked");
for (const path of packedFiles) {
  if (!fileSources.has(path)) fileSources.set(path, "packed");
}
const filesToScan = [...fileSources.keys()].sort();

checkPackedFiles(packedFiles);
checkLocalEvidenceFiles(trackedFiles, untrackedFiles, packedFiles);
const dependencyLicenseCount = checkLicenses(packageJson, lockJson);
for (const path of filesToScan) scanFile(path, fileSources.get(path) ?? "unknown");
